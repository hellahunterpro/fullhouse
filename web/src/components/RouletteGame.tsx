...
export function RouletteGame({ balance, onBalanceUpdate }: Props) {
  ...
  return (
    <div style={{ padding: '16px' }}>
      ...
      <div style={{ background: tokens.bgSecondary, borderRadius: tokens.radius, padding: '20px', marginBottom: '16px' }}>
        <label style={{ color: tokens.textSecondary, fontSize: '14px', display: 'block', marginBottom: '12px' }}>Bet Type</label>
          ...
          {(Object.keys(BET_LABELS) as BetType[]).map((bt) => (
            <button key={bt} onClick={() => setBetType(bt)}
              style={{
                ...
                background: betType === bt ? tokens.accent : tokens.bg,
        ...
        {betType === 'straight' && (
          <div style={{ marginTop: '12px' }}>
            <label style={{ color: tokens.textSecondary, fontSize: '14px' }}>Number: {straightNumber}</label>
      ...
      <div style={{ background: tokens.bgSecondary, borderRadius: tokens.radius, padding: '16px', marginBottom: '16px', display: 'flex', justifyContent: 'space-between' }}>
        ...
        <div><div style={{ color: tokens.textSecondary, fontSize: '12px' }}>Payout</div><div style={{ color: tokens.success, fontSize: '18px', fontWeight: 'bold' }}>{(stake * multiplier).toLocaleString()}</div></div>
      </div>
